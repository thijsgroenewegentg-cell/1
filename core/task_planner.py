...
def create(title: str, steps: list[str | dict], owner: str = "user") -> dict:
    clean: list[dict] = []
    for raw in steps:
        if isinstance(raw, dict):
            ...
            dependencies = []
        if text:
            clean.append({"text": text[:300], "depends_on": dependencies[:8]})
    if not clean:
    ...
    plan = {
        ...
        "cursor": 0,
        "steps": [
            {"index": i + 1, "text": item["text"], "depends_on": item["depends_on"], "status": "pending", "started": "", "finished": ""}
...
def update(index: int, status: str, note: str = "") -> dict | None:
    ...
    with _lock:
        ...
        if status == "running":
            blocked = [dep for dep in step.get("depends_on", []) if not any(
                candidate.get("index") == dep and candidate.get("status") in {"done", "skipped"}
                for candidate in plan.get("steps", [])
            )]
            if blocked:
                raise ValueError(f"Step {index} is waiting for dependencies: {', '.join(map(str, blocked))}.")
...
def render(plan: dict | None = None) -> str:
    ...
    for step in plan.get("steps", []):
        ...
        deps = f" (after {','.join(map(str, step.get('depends_on', [])))})" if step.get("depends_on") else ""
        lines.append(f"{icon} {step['index']}. {step['text']}{deps} [{step['status']}]" + note)
    return "\n".join(lines)
