...
def _get_api_key() -> str:
    """Compatibility stub: the local build never reads a cloud key."""
    return ""


def _get_local_model(model=None):
    from core.llm_client import Model
    return Model(model=None, num_predict=4096)
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    The local model. If the API is unreachable, it falls back to language-agnostic
    ...
    if desc:
        try:
            ...
            ans = _get_local_model().generate_content(prompt).text.strip().lower()
            ans = ans.strip("`'\". \n")
...
Please:
1. Identify errors or problems visible on screen
2. Explain the cause simply
3. Give a concrete fix
4. If code is visible, show corrected code

Be specific and actionable. Quote visible error messages exactly."""
        analysis = call_llm_text(
            analysis_prompt, images=[image_bytes], num_predict=1200, timeout=300
        )
