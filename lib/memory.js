...
let memories = null; // [{fact, at, profile}]

function normalizeProfile(p) {
  const s = String(p || 'main').trim().toLowerCase().replace(/[^a-z0-9_-]/g, '');
  return s.slice(0, 20) || 'main';
}
...
function all(profile) {
  ...
  return (p ? list.filter((m) => (m.profile || 'main') === p) : list)
    .map((m, i) => ({ idx: i, fact: m.fact, at: m.at, profile: m.profile || 'main' }));
}

function add(fact, profile) {
  ...
  list.push({ fact: clean, at: new Date().toISOString(), profile: prof });
  save();
...
function resetCache() { memories = null; }
 ...
 * scoped to a profile, falling back to the most recent ones when embeddings
 * aren't available.
 * @returns {Promise<string[]>} selected facts
 */
async function relevantMemories(ollamaUrl, queryText, k = 20, profile) {
  const prof = normalizeProfile(profile);
  const list = load().filter((m) => (m.profile || 'main') === prof);
  if (list.length === 0) return [];
...
module.exports = { all, add, removeContaining, clear, promptSection, relevantMemories, resetCache, normalizeProfile };
