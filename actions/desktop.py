...
def _ask_local_for_desktop_action(task: str) -> str:
    """Ask the configured local Ollama model for a safe desktop snippet."""
    from core.llm_client import call_llm_text

    desktop = str(_get_desktop())
    os_specific = (
        "- ctypes (Windows API calls, read-only)\n- winreg (registry READ only)"
        if _OS == "Windows" else "- pathlib.Path for inspection only"
    )
...
Allowed modules ONLY: pyautogui, pathlib.Path, shutil.copy2/copytree/disk_usage, time.sleep.
{os_specific}

Hard rules: no deletion, no subprocess, no exec/eval, no imports, and no writes
unless explicitly requested. If unsafe, output exactly UNSAFE.
Output ONLY Python code.

Task: {task}"""
    try:
        return call_llm_text(prompt, num_predict=700, timeout=180).strip()
    except Exception as e:
        print(f"[Desktop] Local model failed: {e}")
        return "UNSAFE"
