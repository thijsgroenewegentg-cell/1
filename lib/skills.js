...
async function execute(skillName, args) {
  ...
  // {{param}} or {{param|default}} — values are URL-encoded.
  const fill = (template) => String(template).replace(/\{\{\s*([\w.]+)(?:\|([^}]*))?\s*\}\}/g, (_m, key, dflt) => {
    const value = args && args[key] != null && String(args[key]).length > 0 ? String(args[key]) : String(dflt || '');
