...
class ControlCenterOverlay(_HudOverlay):
    ...
    def __init__(self, command=None, timeline_getter=None, statuses_getter=None, parent=None):
        ...
        self._tabs.addTab(self._build_runtime_tab(), "RUNTIME")
    ...
    def _refresh_memory(self, *_):
        ...
        for row in self._memory_rows:
            ...
            stale = " · STALE (not used in prompts)" if row.get("stale") else ""
            provenance = f" · scope {row.get('scope', category)} · source {row.get('source', 'unknown')}"
            if row.get("confidence"):
                provenance += f" · confidence {row['confidence']}"
            lines.append(f"{category}/{key} · {row.get('updated', '—')}{expiry}{stale}{provenance}\n  {row.get('value', '')}")
    ...
    def _build_runtime_tab(self) -> QWidget:
        w = QWidget(); lay = QVBoxLayout(w); lay.setContentsMargins(8, 8, 8, 8); lay.setSpacing(6)
        self._runtime_box = self._box(190); lay.addWidget(self._runtime_box, 1)
        row = QHBoxLayout(); row.setSpacing(4)
        row.addWidget(self._button("TASK TRACE", lambda: self._invoke("task_runtime", {"action": "status", "history": True}), True))
        row.addWidget(self._button("TIMELINE", lambda: self._invoke("task_runtime", {"action": "timeline"})))
        row.addWidget(self._button("ALERTS", lambda: self._invoke("proactive", {"action": "list"})))
        row.addWidget(self._button("POLICY", lambda: self._invoke("approval_policy", {"action": "status"})))
        lay.addLayout(row)
        profile_row = QHBoxLayout(); profile_row.setSpacing(4)
        self._approval_profile = QComboBox(); self._approval_profile.addItems(["safe_read_only", "confirm_once_per_task", "always_confirm"]); profile_row.addWidget(self._approval_profile, 1)
    ...
    def _set_approval_profile(self, *_):
        self._invoke("approval_policy", {"action": "set_profile", "profile": self._approval_profile.currentText()})
    ...
    def _build_blender_tab(self) -> QWidget:
        ...
        for label, action in (("STATUS", "status"), ("PLAN", "plan"), ("VISUAL REVIEW", "visual_review"), ("OBJECTS", "list_objects"), ("UNDO", "undo"), ("CHECKPOINT", "scene_checkpoint"), ("RENDER PREVIEW", "render")):
    ...
    def refresh(self):
        ...
        try:
            from core.task_runtime import render_runtime
            from core.approval_policy import describe_policy, get_profile
            from core.proactive_state import list_alerts
            if hasattr(self, "_approval_profile"):
                self._approval_profile.setCurrentText(get_profile())
            alerts = list_alerts(unread_only=True, limit=4)
            alert_text = "\n".join(f"{a.get('id')} · {a.get('message')}" for a in alerts) or "No unread proactive alerts."
            self._runtime_box.setPlainText(render_runtime(include_history=True) + "\n\n" + describe_policy() + "\n\nUNREAD ALERTS\n" + alert_text)
        except Exception as exc:
            self._runtime_box.setPlainText(f"Runtime unavailable: {exc}")
