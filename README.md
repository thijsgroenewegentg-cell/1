...
The remote dashboard needs the optional packages in `requirements.txt` (`fastapi`, `uvicorn`, `cryptography`, `python-multipart` and `qrcode[pil]`). If they were skipped during installation, run `pip install -r requirements.txt` and restart MARK. The dashboard is intended for a trusted local network; use the HTTPS certificate files in `config/certs/` if you need encrypted transport on the LAN. Its task center mirrors preflight, active tool, confirmation, verification and recovery events and exposes bounded pause, resume, rollback-plan, recovery and cancel controls; these controls never replay a mutation.

The main HUD uses a black, low-distraction particle-orb visualizer: a depth-shaded dotted sphere, restrained blue/indigo glow and compact instrument readouts. The orb keeps the existing microphone/TTS amplitude response, listening/thinking/speaking/muted states, confirmation overlays and accessibility controls, while the surrounding panels remain available for chat, logs and settings. The header status strip reports Ollama, microphone, Edge TTS, vision, Blender, optional ComfyUI and Internet degradation states. **◈ CONTROL CENTER** opens dedicated Task Plan, Timeline, Memory, Workflows and Blender tabs with safe pause/continue/rollback, forget, workflow editing/replay, checkpoints and render/undo controls. Click or focus the orb and press Space/Enter to interrupt a response; when idle, the same control toggles the microphone. During MARK speech, the strip explicitly shows that voice input can barge in and stop generation. The command panel also provides a hands-free/push-to-talk selector: in push-to-talk mode, hold **HOLD TO TALK** while speaking and release to send; background audio is not transcribed.

MARK keeps a small project-scoped task journal in ignored `memory/projects/` files. It records bounded operation metadata and recent session summaries, never source files or credentials, and includes the active project inventory in the local prompt. Complex work uses a visible inspect → plan → confirm → act → verify → recover checklist; tool results update the checklist and durable operation checkpoints so a failed action is reported with a safe recovery path.
...
MARK uses the existing **Blender MCP** add-on; it does not require another Blender add-on. You can ask MARK to **connect to Blender**: it opens or focuses Blender, presses `N`, selects the existing Blender MCP panel, activates its connection control, starts the fixed local MCP client, and verifies `tools/list`. If the UI step needs help, in Blender's 3D View sidebar set the port to `9876` and click **Connect to MCP server**. In MARK's **PLUGIN SETTINGS → BLENDER — EXISTING MCP SERVER**, keep host `127.0.0.1`, port `9876`, and launcher `uvx`. MARK starts the fixed `uvx blender-mcp` stdio server, discovers safe advertised schemas in the background, and exposes them to Ollama as first-class `blender_mcp_*` tools instead of making the model guess a nested call envelope. Unknown or dangerous code/command tools are filtered out. Read-only inspection is immediate; each mutation captures scene context, waits for the normal confirmation card, executes once, and verifies with a fresh scene snapshot. Screenshot/render image results are passed to the configured local vision model for bounded visual feedback, and a render without an image result gets one best-effort read-only viewport check. The `blender/mark_bridge.py` file is an optional older MARK bridge and is not needed for the BlenderMCP add-on shown in the panel.

## Language and voice

English and Dutch are first-class MARK languages. In **⚙ → CUSTOMISE ASSISTANT → LANGUAGE**, choose **Automatic / Automatisch**, **English**, or **Nederlands**. Automatic mode detects the latest typed/recognized turn locally; Dutch mode also configures faster-whisper with `nl` and uses native Edge voices (`nl-NL-FennaNeural`, `nl-NL-ColetteNeural` or `nl-NL-MaartenNeural`). Prompts, confirmations/status guidance, desktop labels and the authenticated remote dashboard follow the active language where the interface has a fixed label. Tool names, JSON keys and safety contracts stay language-neutral.

Ollama returns text, so MARK uses `edge-tts` for natural spoken replies without an API key. It needs internet access for synthesis. If Edge TTS is unavailable, MARK tries the local `pyttsx3` system voice. Microphone transcription uses `faster-whisper` and downloads its model once on first voice use; typed commands work without a microphone. Speech is sentence-streamed and interruptible: microphone barge-in or the orb/remote interrupt stops the current TTS response without silently executing the remainder.

If MARK does not hear you, open **⚙ → AUDIO DEVICES**, select the actual microphone, press **TEST MICROPHONE**, or use **RECORD 5s + PLAYBACK** for an end-to-end check, then press **APPLY**. The diagnostics panel shows the live level, peak, frame count, PortAudio device/index, sample rate, channel count and stream status. Applying now reconnects the live microphone stream. Also check that the HUD button says **MICROPHONE ACTIVE**, and that your operating system has granted MARK microphone permission. The Activity Log will show the PortAudio/device error when the stream cannot open. For noisy rooms or speaker bleed, use **PUSH-TO-TALK ENABLED** and hold **HOLD TO TALK**; hands-free mode still uses sustained, thresholded barge-in rather than a single hot frame.
...
- `core/project_context.py` — secret-safe project inventory used by the scoped task-memory prompt
...
### Task runtime, approvals and local evaluation

The runtime now keeps a durable, inspectable task envelope in ignored `memory/task_runtime.json`: the original request, bounded task type/intent/language metadata, resumable conversation buffer, clarification questions, operation status, verification and recovery notes are persisted. A task in `needs_input` or `paused` survives restart; a pending confirmation is reconciled into a fresh `needs_input` review because its process-local callback cannot be restored, while an active task is reconciled as `interrupted`. Resume never replays a previous operation. Visible plans can declare dependencies, interface-issued approvals are recorded separately from model arguments, and every tool outcome is normalized to `ok`, `summary`, `changed_items`, `verification`, `reversible` and `recovery`. Only read-only transient failures can be retried once; mutations are never replayed automatically. Use the `task_runtime` and `recovery` tools or the Control Center's **RUNTIME** tab to inspect it.

The **RUNTIME** tab also exposes three approval profiles: `safe_read_only` (default), `confirm_once_per_task`, and `always_confirm`. Profiles add confirmation requirements but never grant arbitrary shell, Python, computer or MCP access. The centralized authority engine records task-scoped grants, inline/deferred approval mode, approval expiry/resolution and an actor-attributed local audit trail in ignored `memory/authority_state.json`. The `authority_control` tool and authenticated dashboard safety controls expose status, audit, emergency pause and emergency kill; clearing an emergency gate is explicit and never replays blocked work. Explicit memory preferences show scope, expiry and provenance, and stale entries are retained for inspection but excluded from current prompt influence. Complex requests receive a deterministic preflight and a bounded inspect → plan → act → verify → recover loop. Set `dry_run` through the `agent_control` action or `config/api_keys.json` to preview mutating calls without executing them; previews include the tool, action, target and recovery guidance. Ollama responses also pass through a bounded tool-output recovery layer: JSON, fenced and slash-prefixed calls are converted only when their names were already advertised, and the structured payload is removed before streaming text reaches the user or TTS.

Blender high-level plans (`plan`, `scene_summary`, `visual_review`) remain bounded descriptions over the existing MCP connection; mutations still use the advertised tool schema, confirmation, scene verification and local visual before/after feedback. Recorded workflows include inputs, a permission manifest, confirmation points, verification and recovery instructions.

Proactive briefings and background alerts are opt-in. MARK persists monitored-topic, project and unfinished-task alerts locally so they can be inspected with the `proactive` tool; it does not add a background network source unless the user configured one. Run the dependency-free local benchmark with:

```bash
python -m evaluation.benchmark
```

It covers tool routing, loopback/code safety, recovery contracts, memory provenance/expiry, Blender planning and hallucination regressions without calling Ollama or executing tools.
