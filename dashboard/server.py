...
def _local_ip() -> str:
    ...
    # Method 4: inspect interfaces when hostname resolution only exposes a
    # link-local address (common in containers and some Wi-Fi setups).
    try:
        import psutil
...
class DashboardServer:
    ...
    def __init__(self):
        ...
        # The dashboard is optional. Do not make importing MARK fail when the
        # optional FastAPI stack is not installed (for example in headless CI).
        ...
        self._uvicorn_servers: list[object] = []
        self._started                     = False
    ...
    def set_command_callback(self, fn) -> None:
        """Receive commands from the browser on the assistant's queue."""
        self._command_callback = fn

    def set_audio_callback(self, fn) -> None:
        """Receive phone microphone PCM16 chunks from the browser."""
    ...
    def available(self) -> bool:
        return bool(_DEPS_OK and self._started)
    ...
    def start(self, timeout: float = 4.0) -> bool:
        """Start the LAN dashboard once, in its own event-loop thread.

        The desktop UI calls this lazily when the user presses Remote Control,
        so MARK does not open a listening port when remote access is unused.
        """
        if not _DEPS_OK:
            self._server_error = (
                "Remote dashboard dependencies are missing. Install fastapi, "
                "uvicorn, cryptography and python-multipart."
            )
            return False
        with self._state_lock:
            if self._started:
        ...
        # Wait for the event loop and uvicorn startup, not just thread creation,
        # so the QR code is useful immediately after the overlay appears.
        if not self._server_ready.wait(timeout=max(0.1, timeout)):
            if self._server_error:
                return False
            # A slow first import may still be finishing. The thread remains
            # alive and a subsequent click can retry without creating another.
            return bool(thread.is_alive() and self._loop_ready.is_set())
    ...
    def _run_server(self) -> None:
        try:
            ...
            print(f"[Dashboard] Server stopped: {exc}")
        finally:
            self._started = False
            self._server_ready.set()

    def stop(self) -> None:
        """Ask uvicorn to stop; used when the desktop assistant exits."""
        with self._state_lock:
            servers = list(self._uvicorn_servers)
            loop = self._loop
    ...
    def publish(self, msg: dict) -> None:
        """Thread-safe broadcast for MARK's worker threads."""
        loop = self._loop
        if not loop or not loop.is_running():
            return

        def _schedule() -> None:
            asyncio.create_task(self.broadcast(dict(msg)))
    ...
    def _queue_command(self, text: str) -> None:
        ...
        if self._command_callback:
            try:
                ...
                print(f"[Dashboard] Command callback failed: {exc}")
    ...
    def _queue_audio(self, data: bytes) -> None:
        if self._audio_callback:
            try:
                ...
                print(f"[Dashboard] Audio callback failed: {exc}")

    def _queue_file(self, path: str, name: str, size: int) -> None:
        if self._file_callback:
            try:
                self._file_callback(path, name, size)
            except Exception as exc:
                print(f"[Dashboard] File callback failed: {exc}")
...
<p>Connecting to JARVIS…</p>
        ...
        async def phone_audio_ws(websocket: WebSocket, token: str = ""):
            ...
            try:
                while True:
                    ...
                    # The assistant's audio bridge performs its own bounded
                    # buffering/VAD. Never block the dashboard event loop on it.
        ...
        if _UPLOAD_OK:
            ...
            async def upload_file(req: Request, file: UploadFile = FastAPIFile(...)):
                ...
                self._queue_file(str(dest), dest.name, size)
    ...
    async def serve(self) -> None:
        ...
        self._loop.run_in_executor(None, _ensure_network_access, PORT)
        ...
        alias_task = None
        if use_ssl:
            alias_task = asyncio.create_task(self._serve_alias())
        ...
        primary = uvicorn.Server(cfg)
        self._uvicorn_servers.append(primary)
        primary_task = asyncio.create_task(primary.serve())
        while not primary.started and not primary_task.done():
        ...
        if primary.started:
            ...
            self._server_error = f"Could not start the dashboard on port {PORT}."
            self._server_ready.set()
