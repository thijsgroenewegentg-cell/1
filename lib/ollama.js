...
const PREFERRED_MODELS = ['mistral-small3.2', 'mistral-small3', 'mistral-small', 'qwen3', 'gemma3', 'llama3.1', 'llama3', 'qwen2.5', 'qwen', 'mistral', 'gemma2', 'gemma', 'phi3', 'deepseek-r1'];
...
function pickDefaultModel(models) {
  // Prefer non-coder models for general "smart" duty.
  const nonCoder = models.filter((m) => !/coder/i.test(m));
  const pool = nonCoder.length > 0 ? nonCoder : models;
  for (const pref of PREFERRED_MODELS) {
    const hit = pool.find((m) => m.toLowerCase().startsWith(pref));
    if (hit) return hit;
  }
  return pool[0] || null;
}
...
async function* streamOllamaChat({ ollamaUrl, model, messages, temperature = 0.7, tools }) {
  ...
  // NOTE: think:true conflicts with tool calling in many Ollama builds — only
  // enable it when no tools are being sent (pure conversation).
  if (THINKING_MODEL_RE.test(String(model)) && (!tools || tools.length === 0)) body.think = true;
