...
async function* agentChat({ ollamaUrl, model, messages, temperature, toolsEnabled, shellAllowed, systemPrompt, requestApproval }) {
  const tools = toolsEnabled ? toolSpecs({ shellAllowed }) : undefined;
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (const call of toolCalls) {
      ...
      const result = await executeTool(name, args, { shellAllowed, ollamaUrl });
  ...
  yield { type: 'error', error: 'tool loop limit reached (6 rounds) — answer with what you have' };
}
