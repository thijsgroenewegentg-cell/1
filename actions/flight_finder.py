...
# English fast-path only — the local model (below) normalizes date expressions in ANY
...
def _parse_date(raw: str) -> str:
    ...
    try:
        ...
        result = call_llm_text(
            f"Today is {today.strftime('%Y-%m-%d')}. Convert this date expression "
            f"to YYYY-MM-DD: {raw!r}. Return only the date.",
            num_predict=30, timeout=90,
        ).strip()
        if re.match(r"\d{4}-\d{2}-\d{2}", result):
            return result[:10]
    except Exception as e:
        print(f"[FlightFinder] local date parse failed: {e}")
...
def _parse_flights_locally(
    ...
    """Extract flight rows with the local model (name kept for compatibility)."""
    from core.llm_client import call_llm_text
    prompt = (
        f"Extract flight options from {origin} to {destination} on {date} from this page text:\n"
        f"{raw_text[:12000]}\n\nReturn ONLY a JSON array of up to five objects with keys "
        "airline, departure, arrival, duration, stops, price, currency. If none, return []."
    )
    try:
        text = call_llm_text(
            prompt,
        ...
        print(f"[FlightFinder] local parse failed: {e}")
        return []
