...
const DEFAULTS = {
  ...
  telegramToken: '',    // Telegram bot token (bridging, optional)
  telegramUrl: '',      // Telegram API base override (testing)
  telegramChatIds: [],   // paired Telegram chats
  sdUrl: '',            // Stable Diffusion server (Automatic1111 / Forge / SD.Next)
  contextLength: 8192,  // num_ctx sent to Ollama (0 = model default)
  keepAlive: '30m',     // keep models loaded ('0' = Ollama default 5m, '-1' = forever)
  ...
  models: { fast: '', smart: '', vision: '', coder: '' },  // '' = auto-detect
  mcps: [],             // MCP servers: [{name, command, enabled}] — e.g. {name:'blender', command:'uvx blender-mcp'}
...
function sanitize(cfgObj) {
  ...
  if (out.models) {
    for (const k of ['fast', 'smart', 'vision', 'coder']) out.models[k] = String(out.models[k] || '').slice(0, 100);
  if (!Array.isArray(out.mcps)) out.mcps = [];
  out.mcps = out.mcps.filter((m) => m && m.name && m.command).slice(0, 8).map((m) => ({
    name: String(m.name).trim().slice(0, 30),
    command: String(m.command).trim().slice(0, 300),
  ...
  out.telegramToken = String(out.telegramToken || '').trim().slice(0, 200);
  out.telegramUrl = /^https?:\/\/.+/i.test(String(out.telegramUrl || '')) ? String(out.telegramUrl).trim().slice(0, 300) : '';
  if (!Array.isArray(out.telegramChatIds)) out.telegramChatIds = [];
  out.sdUrl = /^https?:\/\/.+/i.test(String(out.sdUrl || '')) ? String(out.sdUrl).trim().slice(0, 300) : '';
  out.contextLength = Math.min(Math.max(parseInt(out.contextLength, 10) || 0, 0), 131072);
  out.keepAlive = /^-?\d+(s|m|h)?$/.test(String(out.keepAlive || '').trim()) ? String(out.keepAlive).trim().slice(0, 10) : '30m';
  if (out.briefing) {
