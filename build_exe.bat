...
REM Fixed to use script directory, not current dir

setlocal
REM Change to script's own directory, so it works no matter where you double-click from
cd /d "%~dp0"
echo Current dir: %cd%
...
REM Check if in jarvis folder - use script dir
...
REM Activate venv if exists - use script dir
...
if not exist "%~dp0data\piper_models\en_GB-alan-medium.onnx" (
...
if exist "%~dp0JARVIS.spec" (
    pyinstaller "%~dp0JARVIS.spec" --noconfirm --clean
) else (
