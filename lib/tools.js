...
async function toolSpecs({ shellAllowed }) {
  const specs = [
    ...
    },
    {
      ...
      function: {
        name: 'open_app',
        description: 'Open an app or game on the user\'s PC by name — matches their Start menu / installed applications (Steam and Epic games install Start-menu shortcuts, so games work by name). Also deep links: name "spotify" + a search opens the Spotify desktop app searching for it (great with Premium). Examples: open_app {name:"spotify", search:"pink floyd"}, {name:"steam"}, {name:"discord"}, a game name.',
        parameters: {
          type: 'object',
          properties: {
            name: { type: 'string', description: 'App or game name, e.g. spotify, steam, discord, or any installed game' },
            search: { type: 'string', description: 'For spotify: what to search in the desktop app' },
          },
          required: ['name'],
        },
      },
    },
    {
      ...
      function: {
        name: 'play_music',
        description: 'Play music: builds PLAY buttons for the user\'s music services (Spotify web, YouTube, YouTube Music) from a search query. No API keys, no accounts, no cost — the buttons open in a new tab.',
        parameters: {
          type: 'object',
          properties: {
            query: { type: 'string', description: 'What to play: artist, song, album, playlist or mood' },
            service: { type: 'string', description: 'Preferred service: spotify | youtube | ytmusic (default spotify)' },
          },
          required: ['query'],
        },
      },
  ...
  // MCP servers (Blender, filesystem, GitHub…) — every remote tool becomes ours.
  try {
    const mcpTools = await require('./mcp').listTools();
    for (const mt of mcpTools) {
      specs.push({
        type: 'function',
        function: {
          name: mt.name,
          description: '[' + mt.server + ' MCP] ' + mt.description.slice(0, 400),
          parameters: mt.schema,
        },
      });
    }
  } catch { /* MCP unreachable — skip */ }
  return specs;
...
async function executeTool(name, args, ctx) {
  ...
  switch (name) {
    ...
    case 'open_app':        return require('./launcher').openApp(a);
    case 'play_music': {
      const q = String(a.query || '').trim().slice(0, 200);
      if (!q) return { error: 'what shall I play?' };
      const service = ['spotify', 'youtube', 'ytmusic'].includes(String(a.service || '').toLowerCase()) ? String(a.service).toLowerCase() : 'spotify';
      const links = {
        ...
        ytmusic: 'https://music.youtube.com/search?q=' + encodeURIComponent(q),
      };
      const launcher = require('./launcher');
      return { ok: true, query: q, service, links, app_link: launcher.spotifySearchUri(q), note: 'PLAY buttons are shown to the user in the chat, including an IN-SPOTIFY-APP button that opens the desktop app searching for this (Premium-friendly). Mention which service you picked; or use open_app with the spotify search for voice control.' };
    }
    case 'maps': {
    ...
    default: {
      ...
      if (String(name).startsWith('mcp_')) return require('./mcp').callTool(name, a);
